...
// Add the Table class
_namespace.table = function(globs,options){
  ...
  OBJ.draw = function(ctx,window){
    ...
    roundRect(ctx, px+0.5,py+0.5, OBJ.cell_width, _cell_height, 4);
    ...
    roundRect(ctx, px+_tab.up.x+0.5,py+_tab.up.y+0.5, _tab_size, 16-4, 6);
    if(OBJ.selected){
      ctx.fill();
    } else {
      ctx.stroke();
...
_namespace.table.new_table_id = 1;
