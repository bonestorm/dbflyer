...
// Add the Menu class
_namespace.menu = function(globs) {
...
/*
    ctx.globalAlpha = 0.5;
    roundRect(ctx,dismiss.x+0.5,dismiss.y+0.5,dismiss.width,dismiss.height,5);
    ctx.stroke();
   ...
   /*
    ...
    roundRect(ctx,_shown_x+_box.x+0.5,_shown_y+_box.y+0.5,_box.width,_box.height,5);
    ctx.stroke();  
    ctx.fill(); 
    */
    ...
    ctx.save();
    ctx.beginPath();
    openDot(ctx,_shown_x,_shown_y,6,8,(_reverse_options ? 2 : 0));
    ...
    for(var box_ind in _option_boxes){
      ...
      ctx.fillStyle = "#ffffff";
      ctx.lineWidth = 0.5;
      roundRect(ctx,_shown_x+box.x+0.5,_shown_y+box.y+0.5,box.width,box.height,6);
      ctx.fill();
      ctx.stroke();
      */
